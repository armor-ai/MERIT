...
def build_AOLDA_input_version(timed_reviews=None):
    ...
    for apk, reviews in timed_reviews.items():
        ...
        # for each interval, build biterm input
        input_X = []
...
def OLDA_fit(OLDA_input, n_topics, win_size, senti_lex=None):
    phis = {}
    theta = {}
    for apk, item in OLDA_input.items():
        ...
        model.fit(input_X)
        phis[apk] = model.B
        fout = open("../result/topic_words_%s_%s_%s_bjst"%(apk, n_topics, win_size), 'w')
        for t_i, phi in enumerate(phis[apk]):
            ...
            for i, senti_topic_dist in enumerate(phi):
                for j, topic_dist in enumerate(senti_topic_dist):
                    top_words = [dictionary[w_id] for w_id in np.argsort(topic_dist)[:-11:-1]]
                    fout.write('Senti {} Topic {}: {}\n'.format(i, j, ' '.join(top_words)))
                fout.write('\n')
        fout.close()
    return phis
...
def topic_labeling(OLDA_input, apk_phis, phrases, mu, lam, theta, save=True):
    ...
    for apk, item in OLDA_input.items():
        ...
        for t_i, phi in enumerate(phis):
            ...
            topic_label_scores = topic_labeling_(count[t_i], total_count[t_i], label_ids[t_i], sensi_label[t_i], phi[2], mu, lam)
            topic_label_sent_score = topic_label_sent(dictionary, phi[2], rawinput_sent, sent_ids[t_i], sensi_sent[t_i], mu, lam)
            ...
            emerging_label_scores, emerging_sent_scores = topic_detect(rawinput_sent, dictionary, phi[2], phis[t_i-1][2], count[t_i], count[t_i-1], total_count[t_i],
