...
def OLDA_fit(OLDA_input, n_topics, win_size):
    ...
    for apk, item in OLDA_input.items():
        ...
        model = OBTM(n_topics=n_topics, vocab_size=len(dictionary), n_iter=1000,
                      refresh=500, window_size=win_size)
        model.fit(input_X)
        phis[apk] = model.B
        fout = open("../result/topic_words_%s_%s_%s_btm"%(apk, n_topics, win_size), 'w')
